...
function ShowData(){
    ...
    $rate2 = $_POST['rate'];
    $sql = "select d.pro_id,pro_name,sum(d.qty) as qty,d.price,sum(d.qty)*d.price as total,note,cate_name from quotationdetail2 d left join products p on d.pro_id=p.pro_id left join category c on p.cate_id=c.cate_id where d.quo_id='$quo_id2' group by d.pro_id;";
    ...
    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
        ...
        $sqlsum = "select sum((qty*price) + vat) as newamount,rate_id,sum(qty*price) as amount,sum(vat) as vat,kip_amount,us_amount,baht_amount from quotationdetail2 d left join quotation2 i on d.quo_id=i.quo_id where d.quo_id = '$quo_id2' group by rate_id;";
...
$mpdf->Output('ໃບສະເໜີລາຄາສິນຄ້າແບບມີສ່ວນລົດ.pdf','I');
?>