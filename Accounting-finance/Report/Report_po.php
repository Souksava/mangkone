...
function ShowData(){
    ...
    if(isset($_POST['btn'])){
        $sql = "select d.po_id,p.po_id as po2,p.po_date,d.bill,d.pdet_name,d.qty,d.price,d.qty*d.price as total,note,rate_id,p.status,emp_name from podetail d left join po p on p.po_id=d.po_id left join employees e on p.emp_id=e.emp_id left join acc_no a on d.acc_id=a.acc_id left join acc_unit u on a.unit_id=u.unit_id left join acc_property i on u.ppy_id=i.ppy_id where ppy_name='ລາຍຈ່າຍ' and (p.po_date between '$date1' and '$date2' or rate_id='$rate_id');";
    ...
    if(isset($_POST['btnall'])){
        $sql = "select d.po_id,p.po_id as po2,p.po_date,d.bill,d.pdet_name,d.qty,d.price,d.qty*d.price as total,note,rate_id,p.status,emp_name from podetail d left join po p on p.po_id=d.po_id left join employees e on p.emp_id=e.emp_id left join acc_no a on d.acc_id=a.acc_id left join acc_unit u on a.unit_id=u.unit_id left join acc_property i on u.ppy_id=i.ppy_id where ppy_name='ລາຍຈ່າຍ';";
        $result = mysqli_query($link,$sql);
        $Bill = 0;
        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
            $Bill = $Bill + 1 ;
