...
function ShowData(){
    ...
    if(isset($_POST['btn'])){
        ...
        $sqlsum = "select sum(qty*price) as amount_kip,rate_id,sum(vat) as vat,sum(qty*price) + sum(vat) as newamount from cash_receipt c left join receipt r on c.re_id=r.re_id left join employees e on r.emp_id=e.emp_id left join customers s on r.cus_id=s.cus_id left join acc_no a on c.acc_id=a.acc_id left join acc_unit u on a.unit_id=u.unit_id left join acc_property p on u.ppy_id=p.ppy_id where (ppy_name='ລາຍຮັບ' and r.re_date between '$date1' and '$date2') and rate_id='LAK';";
        $resultsum = mysqli_query($link,$sqlsum);
        $rowsum = mysqli_fetch_array($resultsum, MYSQLI_ASSOC);
        $sqlsum2 = "select sum(qty*price) as amount_baht,rate_id,sum(vat) as vat,sum(qty*price) + sum(vat) as newamount from cash_receipt c left join receipt r on c.re_id=r.re_id left join employees e on r.emp_id=e.emp_id left join customers s on r.cus_id=s.cus_id left join acc_no a on c.acc_id=a.acc_id left join acc_unit u on a.unit_id=u.unit_id left join acc_property p on u.ppy_id=p.ppy_id where (ppy_name='ລາຍຮັບ' and r.re_date between '$date1' and '$date2') and rate_id='THB';";
        $resultsum2 = mysqli_query($link,$sqlsum2);
        $rowsum2 = mysqli_fetch_array($resultsum2, MYSQLI_ASSOC);
        $sqlsum3 = "select sum(qty*price) as amount_us,rate_id,sum(vat) as vat,sum(qty*price) + sum(vat) as newamount from cash_receipt c left join receipt r on c.re_id=r.re_id left join employees e on r.emp_id=e.emp_id left join customers s on r.cus_id=s.cus_id left join acc_no a on c.acc_id=a.acc_id left join acc_unit u on a.unit_id=u.unit_id left join acc_property p on u.ppy_id=p.ppy_id where (ppy_name='ລາຍຮັບ' and r.re_date between '$date1' and '$date2') and rate_id='USD';";
        $resultsum3 = mysqli_query($link,$sqlsum3);
        $rowsum3 = mysqli_fetch_array($resultsum3, MYSQLI_ASSOC);
        $sqlsum4 = "select sum(qty*price) as amount,rate_id,sum(vat) as vat,sum(qty*price) + sum(vat) as newamount from cash_receipt c left join receipt r on c.re_id=r.re_id left join employees e on r.emp_id=e.emp_id left join customers s on r.cus_id=s.cus_id left join acc_no a on c.acc_id=a.acc_id left join acc_unit u on a.unit_id=u.unit_id left join acc_property p on u.ppy_id=p.ppy_id where rate_id='$rate_id' and ppy_name='ລາຍຮັບ';";
        ...
        if(trim($rate_id) == ""){
                if(mysqli_num_rows($resultsum) > 0){
                    $output .='
                        ...
                        <tr style="font-size: 26px;">
                            ...
                            <th colspan="3" style="text-align: right;">'.$rowsum["rate_id"].' '.number_format(($rowsum["vat"] + $rowsum["amount_kip"]),2).'</th>
                        </tr>  
                    ';
                }
                if(mysqli_num_rows($resultsum2) > 0){
                    $output .='
                        <tr style="font-size: 26px;">
                        ...
                        <tr style="font-size: 26px;">
                            ...
                            <th colspan="3" style="text-align: right;">'.$rowsum2["rate_id"].' '.number_format(($rowsum2["vat"] + $rowsum2["amount_baht"]),2).'</th>
                        </tr>  
                    ';
                }
                if(mysqli_num_rows($resultsum3) > 0){
                    $output .='
                        <tr style="font-size: 26px;">
                        ...
                        <tr style="font-size: 26px;">
                            ...
                            <th colspan="3" style="text-align: right;">'.$rowsum3["rate_id"].' '.number_format(($rowsum3["vat"] + $rowsum3["newamount"]),2).'</th>
                        </tr>  
                    ';
                }
        }
            ...
            <tr style="font-size: 26px;">
                ...
                <th colspan="6" style="text-align: right;"> '.$rowsum4["rate_id"].' '.number_format($rowsum4["amount"],2).'</th>
            </tr>  
            <tr style="font-size: 26px;">
                ...
                <th colspan="3" style="text-align: right;">'.$rowsum4["rate_id"].' '.number_format(($rowsum4["vat"] + $rowsum4["amount"]),2).'</th>
            </tr>  
        ';
        }
        return $output;
    ...
    if(isset($_POST['btnall'])){
        ...
        $output .='
                ...
                <tr style="font-size: 26px;">
                    <th colspan="5" style="text-align: right;">VAT 10%: ('.$rowsum3["rate_id"].')</th>
                    <th colspan="3" style="text-align: right;">'.$rowsum3["rate_id"].' '.number_format($rowsum3["vat"],2).' </th>
                    <th colspan="3" style="text-align: right;">'.$rowsum3["rate_id"].' '.number_format(($rowsum3["vat"] + $rowsum3["amount_us"]),2).'</th>
                </tr>  
                ';
        return $output;
    }
