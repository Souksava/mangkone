...
function ShowData(){
    ...
    $sqlsum = "select sum((qty*price) + vat) as newamount,rate_id,sum(qty*price) as amount,sum(vat) as vat,kip_amount,baht_amount,us_amount from invoicedetail d left join invoice i on d.voice_id=i.voice_id where d.voice_id = '$voice_id2' group by rate_id;";
    ...
    $output .='
                <tr >
                    <td colspan="2" align="right"><h4><b>Amount ('.$rate2.') </b></h4></td>
                    <td colspan="3" align="right"><h4><b>'.$rate2.' '.number_format($rowsum["amount"],2).'</h4> </b></td>
                </tr>    
                <tr class="fontblack18">
                    <td colspan="2" align="right"><h4><b>VAT 10% ('.$rate2.') </b></h4></td>
                ...
                <tr class="fontblack18">
                    <td colspan="2" align="right"><h4><b>Special Discount ('.$rate2.') </b></h4></td>
                    <td colspan="3" align="right"><h4><b>('.$rate2.' '.number_format($discount,2).')</h4> </b></td>
                </tr>  
                <tr class="fontblack18">
                    <td colspan="2" align="right"><h4><b>Total Amount ('.$rate2.') </b></h4></td>
...
$mpdf->Output('ໃບວາງບິນອິນເຕີເນັດແບບມີສ່ວນລົດ.pdf','I');
?>