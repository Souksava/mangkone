...
function ShowData(){
    ...
    $sqlAmount = "select sum(qty*price) as Total from invoicedetail2 where voice_id='$voice_id2';";
    $result7 = mysqli_query($link, $sqlAmount);
    $rowAmount = mysqli_fetch_array($result7, MYSQLI_ASSOC);
    $sqlvat = "select sum(vat) as vat from invoicedetail2 where voice_id='$voice_id2';";
    $resultvat = mysqli_query($link, $sqlvat);
    $rowvat = mysqli_fetch_array($resultvat, MYSQLI_ASSOC);
    $sqltotalamount = "select sum(vat) + sum(qty*price) as amount from invoicedetail2 where voice_id='$voice_id2';";
    ...
    $output .='
                <tr class="fontblack18">
                    <td colspan="5" align="right"><h3><b>Amount ('.$rate2.') </b></h3></td>
                    <td colspan="2" align="right"><h3><b>'.$rate2.' '.number_format($rowAmount["Total"],2).'</h3> </b></td>
                </tr>    
                <tr class="fontblack18">
                    <td colspan="5" align="right"><h3><b>VAT 10% ('.$rate2.') </b></h3></td>
                    <td colspan="2" align="right"><h3><b>'.$rate2.' '.number_format($rowvat["vat"],2).'</h3> </b></td>
                </tr>    
                <tr class="fontblack18">
                    <td colspan="5" align="right"><h3><b>Total Amount ('.$rate2.') </b></h3></td>
...
$mpdf->Output('ໃບວາງບິນສິນຄ້າແບບບໍ່ມີສ່ວນລົດ.pdf','I');
?>