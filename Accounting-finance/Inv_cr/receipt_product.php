...
<html>
  ...
  <body >
    ...
    <div class="container font16">
        <form action="receipt_product.php" method="POST" id="form1">
    ...
    <?php 
      if(isset($_POST['btnshow'])){
        ...
        $sqlget = "select voice_id,i.emp_id,emp_name,kip_amount,baht_amount,us_amount,in_date,in_time,i.cus_id,company,i.img_path from invoice2 i left join employees e on i.emp_id=e.emp_id left join customers c on i.cus_id=c.cus_id where voice_id='$invoice';";
    ...
    <div class="container font14">
        ...
        <table class="table table-striped">
            ...
            <?php
                $sqltotal = "select kip_amount,baht_amount,us_amount from invoice2 where voice_id='$invoice';";
                ...
                $sqldis = "select (sum(d.qty*d.price) - ((sum((d.qty*d.price) + vat) - (sum((d.qty*d.price) + vat) - '$amountTotal')) / 1.1)) / count(d.qty) as discount from invoicedetail2 d left join invoice2 i on d.voice_id=i.voice_id left join products p on d.pro_id=p.pro_id where d.voice_id='$invoice';";
                $resultdis = mysqli_query($link,$sqldis);
                $rowdis = mysqli_fetch_array($resultdis,MYSQLI_ASSOC);
                $discount = $rowdis['discount'];
                $sql = "select indet_id,d.pro_id,serial,mac_address,pro_name,d.qty,d.price - ('$discount' / d.qty) as price,(d.price - ('$discount' / d.qty)) * d.qty as total,((d.price - ('$discount' / d.qty)) * d.qty) * 0.10 as vat,d.voice_id,rate_id,note from invoicedetail2 d left join invoice2 i on d.voice_id=i.voice_id left join products p on d.pro_id=p.pro_id where d.voice_id='$invoice';";
            ...
            <?php
                ...
                $sqlsum = "select sum((d.price - ('$discount' / d.qty)) * d.qty) as amount,(sum((d.price - ('$discount' / d.qty)) * d.qty)) * 0.10 as vat,(sum((d.price - ('$discount' / d.qty)) * d.qty)) + ((sum((d.price - ('$discount' / d.qty)) * d.qty)) * 0.10) as newamount,rate_id from invoicedetail2 d left join invoice2 i on d.voice_id=i.voice_id where d.voice_id='$invoice' group by d.voice_id;";
            ...
            <tr>
                <td colspan="9">
                   <form action="receipt_product.php" method="POST" id="formSave">
    ...
    if(isset($_POST['btnSave'])){
        ...
        if(mysqli_num_rows($resultck) > 0){
            ...
            echo"window.location.href='receipt_product.php';";
            echo"</script>";
        }
            ...
            if($rate_id == "LAK"){
                ...
                if(!$resultinsertreceipt){
                    ...
                    $sqlsave = "insert into cash_receipt(re_id,cash_name,qty,price,vat,cash_date,bill,acc_id,rate_id,rate,barcode) select '$bill' as re_id,pro_name,i.qty,i.price - ('$discount2' / i.qty) as price,((i.price - ('$discount2' / i.qty)) * i.qty) * 0.10 as vat,'$in_date' as cash_date,voice_id,acc_id,rate_id,'$rate',i.pro_id from invoicedetail2 i left join products p on i.pro_id=p.pro_id where voice_id='$voice_id';";
