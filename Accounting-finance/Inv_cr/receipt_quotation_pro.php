...
<html>
  <head>
    ...
    <title>Receipt Quotation Product</title>
  ...
  <body >
    ...
    <div class="container font16">
        <form action="receipt_quotation_pro.php" method="POST" id="form1">
            <div class="row" align="center">
                <div class="col-md-12 form-group">
                    <input type="text" name="quo_id" class="form-control" placeholder="ເລກທີບິນ Quotation">
                </div>
                <div class="col-md-12 form-group">
    ...
    <?php 
      if(isset($_POST['btnshow'])){
        $quo_id = $_POST['quo_id'];
        $sqlget = "select quo_id,i.emp_id,emp_name,kip_amount,baht_amount,us_amount,in_date,in_time,i.cus_id,company,i.img_path from quotation2 i left join employees e on i.emp_id=e.emp_id left join customers c on i.cus_id=c.cus_id where quo_id='$quo_id';";
    ...
    <div class="container font14">
        <div class="row" style="float: left; width: 50%;text-align: left">
            <label>ເລກທີ Quotation: <?php echo $rowget['quo_id'];?></label><br>
            <label>ວັນທີສ້າງ Quotation: <?php echo $rowget['in_date'];?></label><br>
        ...
        <div class="row" style="float: left; width: 50%;text-align: right">
            ...
            <label>ຜູ້ສ້າງ Quotation: <?php echo $rowget['emp_name'];?></label><br>
        ...
        <table class="table table-striped">
            ...
            <?php
                $sqltotal = "select kip_amount,baht_amount,us_amount from quotation2 where quo_id='$quo_id';";
                ...
                $sqldis = "select (sum(d.qty*d.price) - ((sum((d.qty*d.price) + vat) - (sum((d.qty*d.price) + vat) - '$amountTotal')) / 1.1)) / count(d.qty) as discount from quotationdetail2 d left join quotation2 i on d.quo_id=i.quo_id left join products p on d.pro_id=p.pro_id where d.quo_id='$quo_id';";
                $resultdis = mysqli_query($link,$sqldis);
                $rowdis = mysqli_fetch_array($resultdis,MYSQLI_ASSOC);
                $discount = $rowdis['discount'];
                $sql = "select quodet_id,d.pro_id,serial,mac_address,pro_name,d.qty,d.price - ('$discount' / d.qty) as price,(d.price - ('$discount' / d.qty)) * d.qty as total,((d.price - ('$discount' / d.qty)) * d.qty) * 0.10 as vat,d.quo_id,rate_id,note from quotationdetail2 d left join quotation2 i on d.quo_id=i.quo_id left join products p on d.pro_id=p.pro_id where d.quo_id='$quo_id';";
            ...
            <?php
                ...
                $sqlsum = "select sum((d.price - ('$discount' / d.qty)) * d.qty) as amount,(sum((d.price - ('$discount' / d.qty)) * d.qty)) * 0.10 as vat,(sum((d.price - ('$discount' / d.qty)) * d.qty)) + ((sum((d.price - ('$discount' / d.qty)) * d.qty)) * 0.10) as newamount,rate_id from quotationdetail2 d left join quotation2 i on d.quo_id=i.quo_id where d.quo_id='$quo_id' group by d.quo_id;";
                $resultsum = mysqli_query($link,$sqlsum);
                $rowsum = mysqli_fetch_array($resultsum, MYSQLI_ASSOC);
            ?>
            <tr class="font26 danger">
                <td colspan="6" align="right"><b>ຍອດລວມ:</b></td>
                <td colspan="3" align="right"> <?php echo number_format($rowsum['amount'],2);?> <?php echo $rowsum['rate_id']; ?></td>
            </tr>
            <tr class="font26 danger">
                <td colspan="6" align="right"><b>VAT:</b></td>
                <td colspan="3" align="right"> <?php echo number_format($rowsum['vat'],2);?> <?php echo $rowsum['rate_id']; ?></td>
            </tr>
            <tr class="font26 danger">
                <td colspan="6" align="right"><b>ຍອດລວມ:</b></td>
                <td colspan="3" align="right"> <?php echo number_format($rowsum['newamount'],2);?> <?php echo $rowsum['rate_id'];?></td>
            </tr>
            <tr>
                <td colspan="9">
                   <form action="receipt_quotation_pro.php" method="POST" id="formSave">
    ...
    if(isset($_POST['btnSave'])){
        ...
        if(mysqli_num_rows($resultck) > 0){
            ...
            echo"window.location.href='receipt_quotation_pro.php';";
            echo"</script>";
        }
        else if(trim($amount) == 0){
            echo"<script>";
            echo"alert('ກະລຸນາປ້ອນເລກທີ Quotation ໃຫ້ຖືກຕ້ອງ');";
            echo"window.location.href='receipt_quotation_pro.php';";
            echo"</script>";
        }
        else{
            ...
            $bill = $rowno['bill'] + 1;
            if($rate_id == "LAK"){
                $insertreceipt = "insert into receipt(re_id,emp_id,cus_id,re_date,kip_amount,bill) value('$bill','$emp_id','$cus_id','$in_date','$amount','$quo_id');";
                ...
                if(!$resultinsertreceipt){
                    ...
                    $sqlsave = "insert into cash_receipt(re_id,cash_name,qty,price,vat,cash_date,bill,acc_id,rate_id,rate,barcode) select '$bill' as re_id,pro_name,i.qty,i.price - ('$discount2' / i.qty) as price,((i.price - ('$discount2' / i.qty)) * i.qty) * 0.10 as vat,'$in_date' as cash_date,quo_id,acc_id,rate_id,'$rate',i.pro_id from quotationdetail2 i left join products p on i.pro_id=p.pro_id where quo_id='$quo_id';";
            ...
            elseif($rate_id == "USD"){
                $insertreceipt2 = "insert into receipt(re_id,emp_id,cus_id,re_date,us_amount,bill) value('$bill','$emp_id','$cus_id','$in_date','$amount','$quo_id');";
                ...
                $insertreceipt3 = "insert into receipt(re_id,emp_id,cus_id,re_date,baht_amount,bill) value('$bill','$emp_id','$cus_id','$in_date','$amount','$quo_id');";
