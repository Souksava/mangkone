...
<html>
  <head>
    ...
    <title>Receipt Quotation internet</title>
  ...
  <body >
    ...
    <div class="container font16">
        <form action="receipt_quotation_in.php" method="POST" id="form1">
    ...
    <?php 
      if(isset($_POST['btnshow'])){
        $quo_id = $_POST['quo_id'];
        $sqlget = "select quo_id,i.emp_id,emp_name,kip_amount,baht_amount,us_amount,in_date,in_time,i.cus_id,company,i.img_path from quotation i left join employees e on i.emp_id=e.emp_id left join customers c on i.cus_id=c.cus_id where quo_id='$quo_id';";
    ...
    <div class="container font14">
        ...
        <table class="table table-striped">
            ...
            <?php
                $sqltotal = "select kip_amount,baht_amount,us_amount from quotation where quo_id='$quo_id';";
                ...
                $sqldis = "select (sum(d.qty*d.price) - ((sum((d.qty*d.price) + vat) - (sum((d.qty*d.price) + vat) - '$amountTotal')) / 1.1)) / count(d.qty) as discount from quotationdetail d left join quotation i on d.quo_id=i.quo_id left join products p on d.pro_id=p.pro_id where d.quo_id='$quo_id';";
                $resultdis = mysqli_query($link,$sqldis);
                $rowdis = mysqli_fetch_array($resultdis,MYSQLI_ASSOC);
                $discount = $rowdis['discount'];
                $sql = "select quodet_id,pro_name,d.qty,d.price - ('$discount' / d.qty) as price,(d.price - ('$discount' / d.qty)) * d.qty as total,((d.price - ('$discount' / d.qty)) * d.qty) * 0.10 as vat,d.quo_id,rate_id,note from quotationdetail d left join quotation i on d.quo_id=i.quo_id left join acc_product p on d.pro_id=p.pro_id where d.quo_id='$quo_id';";
            ...
            <?php
                ...
                $sqlsum = "select sum((d.price - ('$discount' / d.qty)) * d.qty) as amount,(sum((d.price - ('$discount' / d.qty)) * d.qty)) * 0.10 as vat,(sum((d.price - ('$discount' / d.qty)) * d.qty)) + ((sum((d.price - ('$discount' / d.qty)) * d.qty)) * 0.10) as newamount,rate_id from quotationdetail d left join quotation i on d.quo_id=i.quo_id where d.quo_id='$quo_id' group by d.quo_id;";
            ...
            <tr>
                <td colspan="9">
                   <form action="receipt_quotation_in.php" method="POST" id="formSave">
    ...
    if(isset($_POST['btnSave'])){
        ...
        if(mysqli_num_rows($resultck) > 0){
            ...
            echo"window.location.href='receipt_quotation_in.php';";
            echo"</script>";
        }
            ...
            if($rate_id == "LAK"){
                ...
                if(!$resultinsertreceipt){
                    ...
                    $sqlsave = "insert into cash_receipt(re_id,cash_name,qty,price,vat,cash_date,bill,acc_id,rate_id,rate,barcode) select '$bill' as re_id,pro_name,i.qty,i.price - ('$discount2' / i.qty) as price,((i.price - ('$discount2' / i.qty)) * i.qty) * 0.10 as vat,'$in_date' as cash_date,quo_id,acc_id,rate_id,'$rate',i.pro_id from quotationdetail i left join acc_product p on i.pro_id=p.pro_id where quo_id='$quo_id';";
